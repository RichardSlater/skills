...
def _capture(value: str) -> str:
    return (
        ...
        else value.encode()[:MAX_CAPTURE_BYTES].decode(errors="replace")
        + "\n[TRUNCATED]"
    )
...
def redact(value: str, token: str | None) -> str:
    return (
        _capture(value.replace(token, "[REDACTED_TOKEN]")) if token else _capture(value)
    )
...
def remaining(deadline: float, clock=time.monotonic) -> float:
    ...
    if value <= 0:
        raise TimeoutError("shared Scorecard deadline exceeded")
    return value
...
def working_runtime(preferred: str | None) -> list[str]:
    return [
        ...
        for name in dict.fromkeys(
            x for x in ([preferred] if preferred else []) + list(RUNTIMES) if x
        )
...
def execute(
    ...
    *,
    ...
    for runtime in working_runtime(preferred):
        ...
        command = (
            [runtime, "run", "--rm"]
            + (["-e", "GITHUB_AUTH_TOKEN"] if token else [])
            + [SCORECARD_ARTIFACT["image"], *args]
        )
    ...
    return result(
        ...
        output_path=None,
        error="; ".join(errors)
        or "No local Scorecard or working container runtime found.",
    )
